  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`${GROUP_TABLE_NAME}\`
            (
                ...
                UNIQUE INDEX \`idx-unique-${GROUP_TABLE_NAME}-name\` (\`name\`),
                PRIMARY KEY (\`id\`)
            )`,
    );
    await queryRunner.query(`
    ALTER TABLE \`${GROUP_TABLE_NAME}\`
      ADD CONSTRAINT \`fk-${GROUP_TABLE_NAME}-created_by\`
        FOREIGN KEY (\`created_by\`) REFERENCES \`${ACCOUNT_TABLE_NAME}\` (\`id\`) ON DELETE SET NULL ON UPDATE CASCADE
  `);
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX \`idx-unique-${GROUP_TABLE_NAME}-name\` ON \`${GROUP_TABLE_NAME}\``,
    );
    await queryRunner.query(
